...
#define MIN_LINEAR_PWM 30

int pwm_of_force (double force)
{
  if (force > MIN_LINEAR_FORCE) {
    return min(MAX_PWM, MIN_LINEAR_PWM + (int)((force - MIN_LINEAR_FORCE) / 0.045 ));
  }
  if (force < -MIN_LINEAR_FORCE) {
    return max(-MAX_PWM, -MIN_LINEAR_PWM + (int)((force + MIN_LINEAR_FORCE) / 0.0502 ));
  }
...
int main() {
  ...
  while (1) {
    ...
    printf("Force entered: %lf\n", force);

    int pwm = pwm_of_force(force);
    printf("Corresponding PWM: %d\n", pwm);
