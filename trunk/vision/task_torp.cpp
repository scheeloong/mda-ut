...
#include "common.h"
#include "stdio.h"
// this is very simular to the task_buoy code, except we are looking at cut-out circles
...
/*
  ...
  */  
    ...
    if (ncircles < 2) {
        ...
        printf ("  vision_TORP: Not enough circles.\n");
        return NO_DETECT;
    }
    ...
    if (ncircles > 2) {
        ...
        if (!(flags & _QUIET)) {
            printf ("  vision_TORP: Cut-out Circle Detected.\n");
...
char controller_TORP (vision_in Input, char &state) {
    ...
    vision_out Output;
    vcode = vision_TORP (Input, Output, _INVERT);
    return '/';
};